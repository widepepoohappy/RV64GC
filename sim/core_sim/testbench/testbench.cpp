...
void tick(Vcore* top, VerilatedVcdC* tfp, int clock_cycle) {
    // fronte basso
    top->clk_i = 0;
    top->eval();
...
int main(int argc, char** argv) {
	...
	auto pc_ptr = top->rootp->core__DOT__instr_fd_u__DOT__PCCurr_o;
	cout << "PC POINTER " << pc_ptr << endl;
	
	if (int(top->mem_ar_valid_o) == 1) {
	    last_one = 0;
	    burst_length = int(top->mem_ar_len_o);
	    pc_ptr = int(top->PCCurr_o);
	    burst_size = pow(2,int(top->mem_ar_size_o));
	}
