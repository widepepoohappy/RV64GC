...
void tick(Vinstr_fd* top, VerilatedVcdC* tfp, int clock_cycle) {
    // fronte basso
    top->clk_i = 0;
    top->eval();
...
int main(int argc, char** argv) {
	...
	auto pc_ptr = top->rootp->instr_fd__DOT__PCCurr_o;
	cout << "PC POINTER " << pc_ptr << endl;
	
	if (int(top->rootp->instr_fd__DOT__mem_ar_valid_o) == 1) {
	    last_one = 0;
	    burst_length = int(top->rootp->instr_fd__DOT__mem_ar_len_o);
	    pc_ptr = int(top->rootp->instr_fd__DOT__PCCurr_o);
	    burst_size = pow(2,int(top->rootp->instr_fd__DOT__mem_ar_size_o));
	}
