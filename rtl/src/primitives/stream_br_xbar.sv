...
module stream_br_xbar #(
  ...
  parameter int unsigned SelWidth = unsigned'(NumOut),
  /// Derived parameter, do **not** overwrite!
  ///
  ...
  always_comb begin
    data_o = '0;
    valid_o = '0;
    for (int j = 0; unsigned'(j) < NumOut; j++) begin : gen_out0
      for (int i = 0; unsigned'(i) < NumInp; i++) begin : gen_out1
        if(out_valid[j][i]) begin
          data_o[j] = data_i[i];
          valid_o[j] = 1'b1;
        end
      end
